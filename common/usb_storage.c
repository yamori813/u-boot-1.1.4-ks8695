...
#else
static unsigned char usb_stor_buf[512] __attribute__ ((aligned (0x1000)));
...
#else
} __ATTRIBUTE_PACK__ umass_bbb_cbw_t;
#endif
...
#else
} __ATTRIBUTE_PACK__ umass_bbb_csw_t;
#endif
...
int usb_stor_scan(int mode)
{
	...
	for(i=0;i<USB_MAX_DEVICE;i++) {
		...
		USB_STOR_PRINTF("i=%d dev=0x%x usb_max_devs=0x%x\n",i, dev, usb_max_devs);
#else
		USB_STOR_PRINTF("i=%d\n",i);
#endif
		if(dev==NULL) {
...
int usb_stor_BBB_comdat(ccb *srb, struct us_data *us)
{
...
#ifdef CONFIG_CENTAUR
			printf("cmd[%d]=%02x ", result, srb->cmd[result]);
#else
	...
	if (result < 0)
#ifdef CONFIG_CENTAUR
		USB_STOR_PRINTF("usb_stor_BBB_comdat:usb_bulk_msg error[0x%x]\n",result);
#else
...
int usb_stor_BBB_transport(ccb *srb, struct us_data *us)
{
	...
	if (dir_in)
		...
		pipe = pipeout;
#ifdef CONFIG_CENTAUR
	USB_STOR_PRINTF("DATA phase: datalen=0x%x dir_in=0x%x pipe=0x%x[0x%x]\n", srb->datalen, dir_in, pipe,usb_sndbulkpipe(us->pusb_dev,us->ep_out) );
#endif
...
static void dump_storage_content(ccb *srb)
{
	int i=0,j;
	extern int usb_read_start;

	if(!usb_read_start)			//set when issuing the command(read)
		return;

	for( i=0;i < srb->datalen; i++)
	{
		if( i%16 == 0 ) printf("0x%04x :",i);
		printf("%02x ",*(srb->pdata+i));
		if( i%16 == 15 ) 
		{
			printf("   "); 
			for( j=i-15;j <=i; j++)
...
#ifdef CONFIG_CENTAUR
	if ( usb_get_lun(dev, iface->bInterfaceNumber) )
	{
		USB_STOR_PRINTF("Problems with getting LUN number\n");
		return 0;
	}	 
#endif
