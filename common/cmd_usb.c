...
if( argc == 4 )
{
	...
	strcpy(cmdbuf[0],"fatload");
	strcpy(cmdbuf[1],"usb");
	strcpy(cmdbuf[2],boot_device);
	strcpy(cmdbuf[3],argv[1]);		//addr
	strcpy(cmdbuf[4],filename);		//filename
	
//printf("argv[0]=%s argv[1]=%s argv[2]=%s argv[3]=%s argv[4]=%s\n", cmdbuf[0], cmdbuf[1], cmdbuf[2],cmdbuf[3],cmdbuf[4]);
	v[0]=cmdbuf[0];v[1]=cmdbuf[1];v[2]=cmdbuf[2];v[3]=cmdbuf[3];v[4]=cmdbuf[4];

	do_fat_fsload(cmdtp,0,5,v);

	strcpy(cmdbuf[0],"bootm");
	strcpy(cmdbuf[1],argv[1]);
	v[2]='\0';

	printf ("Automatic boot of image at addr 0x%08lX ...\n", addr);
	rcode=do_bootm (cmdtp, 0, 2, v);
}
#else
...
U_BOOT_CMD(
	...
	"loadAddr dev:part\n"
#else
	usbboot,	4,	1,	do_usbboot,
	"usbboot - boot from USB device\n",
	"loadAddr dev:part <filename>\n"
#endif
);
