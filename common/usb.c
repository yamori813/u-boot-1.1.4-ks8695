...
static int hub_port_reset(struct usb_device *, int, unsigned short *);
...
#ifdef CONFIG_CENTAUR
void disp_content( char *ptr, int len)
{
/*
        int     i=0;
        printf("Descriptor= ");
        len = (len >= 0x30 ) ? 0x30 : len;
        for(i=0;i< len; i++)
        {
                printf("0x%02x ", *(ptr+i));
                if( (i > 1) && (i%15==0) ) printf("\n");
        }
        printf("\n");
*/
}
...
#ifdef CONFIG_CENTAUR
 	USB_PRINTF("usb_control_msg: REQ=0x%02X, type=0x%02X value=0x%04X index= 0x%04X length=0x%04X\n",
#else
...
#ifdef CONFIG_CENTAUR
				USB_PRINTF("if=%d, ep=%d epaddr=0x%x\n",ifno,epno,dev->config.if_desc[ifno].ep_desc[epno].bEndpointAddress );
#else
...
#ifdef CONFIG_CENTAUR
	    interface, NULL, 0, USB_CNTL_TIMEOUT )) < 0)
#else
...
int usb_get_lun(struct usb_device *dev, int ifnum)
{
	int res;
	char	buff[64];
	memset(buff,0x5a,sizeof(buff));
	USB_PRINTF("usb_get_lun\n");
	res=usb_control_msg(dev, usb_rcvctrlpipe(dev,0),
                                 USB_BULK_GET_MAX_LUN,
                                 USB_DIR_IN | USB_TYPE_CLASS |
                                 USB_RECIP_INTERFACE,
                                 //0, us->ifnum, buff, 1, USB_CNTL_TIMEOUT);
                                 0, ifnum , buff, 1, USB_CNTL_TIMEOUT);
	USB_PRINTF("usb_get_lun: max_lun=0x%d res=0x%x ifnum=0x%x\n",buff[0],res, ifnum);
	return (res > 0) ? 0 : 1 ;
}
#endif
...
#ifdef CONFIG_CENTAUR
int usb_string_sub1(struct usb_device *dev, unsigned int langid,
		unsigned int index, unsigned char *buf, unsigned int r_len)
{
	int rc=0;
	...
	/* Try to read the string descriptor by asking for the maximum
	 * possible number of bytes */
	rc = usb_get_string(dev, langid, index, buf, r_len);
...
int usb_string1(struct usb_device *dev, int index, char *buf, size_t size)
{
	unsigned char mybuf[USB_BUFSIZ];
	unsigned char *tbuf;
	int err;
	...
	/* get langid for strings if it's not yet known */
//printf("DC:: dev->have_langid=0x%x\n", dev->have_langid);

	if (!dev->have_langid) {
		//err = usb_string_sub1(dev, 0, 0, tbuf);
		err = usb_string_sub1(dev, 0x0, USB_DT_STRING << 8, tbuf,2);		//read length

		//printf("err=0x%x tbuf=0x%x 0x%x \n", err, tbuf[0], tbuf[1]);

		if (err < 0) {
			USB_PRINTF("error getting string descriptor length (error=%x)\n",dev->status);
			return -1;
		}

		err = usb_string_sub1(dev, 0x0, USB_DT_STRING << 8, tbuf,tbuf[0]);	//read language code
		if (err < 0) {
			USB_PRINTF("error getting language code (error=%x)\n",dev->status);
			return -1;
		}

		dev->have_langid = -1;
		dev->string_langid = tbuf[2] | (tbuf[3]<< 8);		//save language code
		USB_PRINTF("USB device number %d default language ID 0x%x\n",
			dev->devnum, dev->string_langid);
	}

	//err = usb_string_sub1(dev, dev->string_langid, index, tbuf);
	err = usb_string_sub1(dev, dev->string_langid, (USB_DT_STRING << 8) | index, tbuf,02);	//read length of string descriptor	
	if (err < 0) {
		USB_PRINTF("error getting length of string descriptor (error=%x)\n",dev->status);
		return -1;
	}

	err = usb_string_sub1(dev, dev->string_langid, (USB_DT_STRING << 8) | index, tbuf, tbuf[0]);	//read string descriptor
	if (err < 0) {
		USB_PRINTF("error getting string descriptor (error=%x)\n",dev->status);
		return -1;
	}
...
int usb_new_device(struct usb_device *dev)
{
	...
	{
	dev->descriptor.bMaxPacketSize0 = 64;  /* Start off at 8 bytes  */
	dev->maxpacketsize = 3;		/* Default to 8 byte max packet size */
...
#undef NEW_INIT_SEQ
...
#ifdef CONFIG_CENTAUR
	{
	char *p=(char *)&dev->descriptor;
	printf("DESC:0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
	...
	if (err < tmp) {
		...
		disp_content((char*) &dev->descriptor,err);
#endif
	/* correct le values */
...
#ifdef CONFIG_CENTAUR
	len=usb_get_configuration_no(dev,&tmpbuf[0],0);
	disp_content((char*) tmpbuf,len);
#else
...
#ifdef CONFIG_CENTAUR
	USB_PRINTF("new device strings: Mfr=0x%x, Product=0x%x, SerialNumber=0x%x\n",
#else
	...
	{
		dev->descriptor.iManufacturer = 3;
		usb_string1(dev, dev->descriptor.iManufacturer, dev->mf, sizeof(dev->mf));
		//usb_string1(dev, dev->descriptor.iProduct, dev->prod, sizeof(dev->prod));
		//usb_string1(dev, dev->descriptor.iSerialNumber, dev->serial, sizeof(dev->serial));
	}
#endif
	USB_PRINTF("Manufacturer %s\n", dev->mf);
#ifndef CONFIG_CENTAUR
