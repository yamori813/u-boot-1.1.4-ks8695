...
#ifdef I2C_WM8974
/* Write WM8974 configuration register
 *
 * Syntax:
 *	imww {i2c_chip} {addr}{.0, .1, .2} {data in word} 
 */
int do_i2c_mww ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	uchar	chip;
	ulong	addr;
	uint	alen;
    ...
    if(i2c_write(chip, addr++, alen, &wData, 1) != 0) {
        puts ("Error writing the chip.\n");
    }

	return (0);
}
#endif /* #ifdef I2C_WM8974 */
...
extern int  i2s_read ( uchar *, int , ushort ,ushort, ulong	);
extern int  i2s_write ( uchar *, int, ushort, ushort, ulong, ulong, ulong );
extern int  i2s_read_file ( uchar *, ulong, ulong );


/* Transmit waveform data file (in the RIFF waveform format) from {addr}.
 *
 * Syntax:
 *	isw {addr} {repeat}
 */
int do_i2s_mwf ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ulong	addr;
    ulong	repeatCount;
    ulong	fmt;
    ...
    fmt = simple_strtoul(argv[2], NULL, 16);
    repeatCount = simple_strtoul(argv[3], NULL, 16);

    if (i2s_read_file( (char *)addr, fmt, repeatCount ) != 0) {
        puts ("Error sending waveform file.\n");
    }
    else
        puts (" done.\n");
...
/* Transmit waveform data from {addr}.
 *
 * Syntax:
 *	isw {addr} {size} {bits/sample} {fmt} {repeat}
 */
int do_i2s_mw ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ulong	addr;
    ulong	numSample;
    ulong	fmt;
	 ...
	 */
    addr = simple_strtoul(argv[1], NULL, 16);
    numSample = simple_strtoul(argv[2], NULL, 16);
    channels = simple_strtoul(argv[3], NULL, 16);
    bitsPerSample = simple_strtoul(argv[4], NULL, 16);
    fmt = simple_strtoul(argv[5], NULL, 16);
    repeatCount = simple_strtoul(argv[6], NULL, 16);
    ...
    if (i2s_write( (char *)addr, 
                   ...
                   repeatCount ) != 0) 
    {
        puts ("Error sending waveform data.\n");
    }
    else
        puts (" done.\n");
...
/* Receive waveform data to  {addr}.
 *
 * Syntax:
 *	isr {addr} {size} {bits/sample}
 */
int do_i2s_mr ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ulong	addr;
    ulong	numSample;
    ulong	bitsPerSample;
    ...
    if (i2s_read( (char *)addr, 
                  numSample, 
                  (ushort)bitsPerSample, 
                  (ushort)channels, 
                  fmt ) != 0) 
    {
        puts ("Error receiving waveform data.\n");
    }
    else
        puts ("done.\n");
...
extern void i2s_init_txInterrupt ( void );
extern void i2s_init_rxInterrupt ( void );
 ...
 *	isr {addr} {size} {bits/sample}
 */
int do_i2s_int ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ulong	wFlag;
     ...
     * Address is always specified.
    */
    wFlag = simple_strtoul(argv[1], NULL, 16);
...
U_BOOT_CMD(
	imww,	4,	1,	do_i2c_mww,
	"imww    - i2c memory write (word)\n",
	"chip address[.0, .1, .2] value (in word)\n    - memory write to WM8974\n"
);
#endif
...
U_BOOT_CMD(
	...
	"[addr] [fmt] [repeat] \n   - transmit waveform data file from this memory address (fmt=0:I2S timing, 1: left-justified timing)\n"
);
U_BOOT_CMD(
	isw,	7,	1,	do_i2s_mw,
	"isw     - i2s transmit waveform data from memory address \n",
	"[addr] [numSample] [chan] [bits/sample] [fmt] [repeat]\n   - transmit waveform data from this memory address (fmt=0:I2S timing, 1: left-justified timing)\n"
);
U_BOOT_CMD(
	isr,	6,	1,	do_i2s_mr,
	"isr     - i2s receive waveform data to the memory address \n",
	"[addr] [numSample] [chan] [bits/sample] [fmt]\n   - receive waveform data to this memory address\n"
);
#ifdef CONFIG_USE_IRQ
U_BOOT_CMD(
	isint,	2,	1,	do_i2s_int,
	"isint    - test i2s interrupt \n",
	"[flag] \n   - test i2s Tx/Rx interrupt (flag=0:Tx; 1:Rx)\n"
);
#endif
