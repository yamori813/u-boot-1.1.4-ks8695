...
ks8695p_run_config	\
ks8695p_config	:	unconfig
	@if [ "$(findstring _run_,$@)" ]; then \
		echo "... configured to run from U-Boot" ; \
		echo "#define CONFIG_SKIP_LOWLEVEL_INIT" >>include/config.h ; \
		echo "#define CONFIG_SKIP_RELOCATE_UBOOT" >>include/config.h ; \
		echo "CONFIG_SKIP_LOWLEVEL_INIT=y" >board/micrel/config.tmp ; \
	else \
		>include/config.h ; \
	fi
	@./mkconfig -a ks8695p arm arm920t ks8695p micrel ks8695

ks8695x_run_config	\
ks8695x_config	:	unconfig
	@if [ "$(findstring _run_,$@)" ]; then \
		echo "... configured to run from U-Boot" ; \
		echo "#define CONFIG_SKIP_LOWLEVEL_INIT" >>include/config.h ; \
		echo "#define CONFIG_SKIP_RELOCATE_UBOOT" >>include/config.h ; \
	...
	@./mkconfig -a ks8695x arm arm920t ks8695x micrel ks8695
...
ks8695v_config	:	unconfig
	...
	@./mkconfig -a ks8695v arm arm920t ks8695v micrel ks8695

ks8695m_config	:	unconfig
	@./mkconfig $(@:_config=) arm arm920t ks8695m micrel ks8695

ks8692_run_config	\
ks8692_samsung_166_config \
ks8692_samsung_200_config \
ks8692_micron_166_config \
ks8692_micron_200_config \
ks8692_config	:	unconfig
	@if [ "$(findstring _run_,$@)" ]; then \
		echo "... configured to run from U-Boot" ; \
		echo "#define CONFIG_SKIP_LOWLEVEL_INIT" >>include/config.h ; \
		echo "#define CONFIG_SKIP_RELOCATE_UBOOT" >>include/config.h ; \
		...
		if [ "$(findstring _samsung_166,$@)" ]; then \
			echo "... configured to use Samsung 166 MHz DDR" ; \
			echo "#define SAMSUNG_166_DDR" >>include/config.h ; \
		elif [ "$(findstring _samsung_200,$@)" ]; then \
			echo "... configured to use Samsung 200 MHz DDR" ; \
			echo "#define SAMSUNG_200_DDR" >>include/config.h ; \
		elif [ "$(findstring _micron_166,$@)" ]; then \
			echo "... configured to use Micron 166 MHz DDR" ; \
			echo "#define MICRON_166_DDR" >>include/config.h ; \
		else \
			echo "... configured to use Micron 200 MHz DDR" ; \
			echo "#define MICRON_200_DDR" >>include/config.h ; \
		fi \
	fi
	@./mkconfig -a ks8692 arm arm920t ks8692 micrel ks8692

ks8692n_run_config	\
ks8692n_samsung_166_config \
ks8692n_samsung_200_config \
ks8692n_micron_166_config \
ks8692n_micron_200_config \
ks8692n_config	:	unconfig
	@if [ "$(findstring _run_,$@)" ]; then \
		echo "... configured to run from U-Boot" ; \
		echo "#define CONFIG_SKIP_LOWLEVEL_INIT" >>include/config.h ; \
		echo "#define CONFIG_SKIP_RELOCATE_UBOOT" >>include/config.h ; \
	...
	@./mkconfig -a ks8692n arm arm920t ks8692n micrel ks8692
...
clean:
	...
	rm -f tools/code_convert
	rm -f board/cray/L1/bootscript.c board/cray/L1/bootscript.image
	rm -f board/netstar/eeprom board/netstar/crcek
	rm -f board/netstar/*.srec board/netstar/*.bin
	rm -f board/trab/trab_fkt board/voiceblue/eeprom
	-rm -f board/integratorap/u-boot.lds board/integratorcp/u-boot.lds board/realview_eb/u-boot.lds
	...
	find include/configs -lname \*realview\*  | xargs rm -fr
