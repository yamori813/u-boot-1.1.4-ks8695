...
static inline void flash_reset ( vu_char* addr )
{
...
unsigned long flash_init (void)
{
	...
	for (i = 0; i < CFG_MAX_FLASH_BANKS; i++) {
		switch (i) {
		case 0:
			flash_get_size ((vu_char *) PHYS_FLASH_1, &flash_info[i]);
...
int flash_erase (flash_info_t * info, int s_first, int s_last)
{
	...
	while (( *addr & 0x80 ) != 0x80 ) {
		if ((now = get_timer(start)) > CFG_FLASH_ERASE_TOUT * prot ) {
			printf ("Timeout\n");
			flash_reset (( vu_char* ) info->start[0]);
			return 1;
		}
...
{
	vu_char* addr = ( vu_char* ) info->start[0];
	vu_char* addr_dest = ( vu_char* ) dest;
	int flag;
	ulong start;
