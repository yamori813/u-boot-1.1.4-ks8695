...
static 
#endif
int irqvec = -1;            /* irq vector, if -1 uhci is stopped / reseted */
...
#else
volatile static uhci_td_t td_int[8] __attribute__((aligned (0x10)));        /* Interrupt Transfer descriptors */
volatile static uhci_qh_t qh_cntrl __attribute__((aligned (0x10)));         /* control Queue Head */
volatile static uhci_qh_t qh_bulk __attribute__((aligned (0x10))) ;          /*  bulk Queue Head */
volatile static uhci_qh_t qh_end __attribute__((aligned (0x10)));           /* end Queue Head */
volatile static uhci_td_t td_last __attribute__((aligned (0x10)));          /* last TD (linked with end chain) */
/* temporary tds */
volatile static uhci_td_t tmp_td[USB_MAX_TEMP_TD] __attribute__((aligned (1024)));          /* temporary bulk/control td's  */
volatile static uhci_td_t tmp_int_td[USB_MAX_TEMP_INT_TD] __attribute__((aligned (1024)));  /* temporary interrupt td's  */
#endif
...
#define PCI_IO_VADDR            0x00000000

#define __io(a)                 (PCI_IO_VADDR + (a))

#define out8r(p,v)      __raw_writeb(v,__io(p))
#define out16r(p,v)     __raw_writew(cpu_to_le16(v),__io(p))
#define out32r(p,v)     __raw_writel(cpu_to_le32(v),__io(p))
#define in8r(p)         ({ unsigned int __v = __raw_readb(__io(p)); __v; })
#define in16r(p)        ({ unsigned int __v = le16_to_cpu(__raw_readw(__io(p))); __v; })
#define in32r(p)        ({ unsigned int __v = le32_to_cpu(__raw_readl(__io(p))); __v; })

extern int flag_response_disp;
...
void disp_offset(char *ptr)
{
        unsigned long *temp;
/*
        printf("*** %s ***\n", ptr ? ptr : "");
printf("DC: framelist=0x%x td_init=0x%x td_last=0x%x, qh_end=0x%x \nqh_bulk=0x%x qh_cntrl=0x%x tmp_init_td=0x%x\n",
        &framelist, &td_int[0], &td_last, &qh_end, &qh_bulk, &qh_cntrl, tmp_int_td);

        temp = (unsigned long *)&framelist;
        printf("framelist = 0x%08x 0x%08x 0x%08x 0x%08x\n", *temp, *(temp+1), *(temp+2), *(temp+3));
        temp = (unsigned long *)&td_int[1];
        printf("td_int[1] = 0x%08x 0x%08x 0x%08x 0x%08x\n", *temp, *(temp+1), *(temp+2), *(temp+4));
        temp = (unsigned long *)&td_int[0];
        printf("td_int[0] = 0x%08x 0x%08x 0x%08x 0x%08x\n", *temp, *(temp+1), *(temp+2), *(temp+4));
        temp = (unsigned long *)&qh_cntrl;
        printf("qh_ctrl = 0x%08x 0x%08x 0x%08x 0x%08x\n", *temp, *(temp+1), *(temp+2), *(temp+4));

        temp = (unsigned long *)&qh_bulk;
        printf("qh_bulk = 0x%08x 0x%08x 0x%08x 0x%08x\n", *temp, *(temp+1), *(temp+2), *(temp+4));

        temp = (unsigned long *)&qh_end;
        printf("qh_end = 0x%08x 0x%08x 0x%08x 0x%08x\n", *temp, *(temp+1), *(temp+2), *(temp+4));

        temp = (unsigned long *)&td_last;
        printf("td_last = 0x%08x 0x%08x 0x%08x 0x%08x\n", *temp, *(temp+1), *(temp+2), *(temp+4));
*/
}
...
unsigned long usb_uhci_td_stat(unsigned long status)
{
	...
	static unsigned long old_status;
...
/*
	if( (old_status != (status & 0x00ff0000) ) && status & TD_CTRL_NAK )
	{
		printf("DC::status=0x%x[0x%x] USBSTS=0x%x USBCMD=0x%x\n",status, old_status,
			in16r(usb_base_addr + USBSTS), in16r(usb_base_addr + USBCMD));
	}

	old_status = status & 0x00ff0000;
*/
...
int usb_get_td_status(uhci_td_t *td,struct usb_device *dev)
{
	...
	do {
		...
		if(stat) {           /* status no ok */
			...
			if ( flag_response_disp )
			{
				//printf("DC:: stat=0x%x status=0x%x info=0x%x td_tmp=0x%08x\n", stat, mytd->status, mytd->info, mytd);
			}
			dev->status=stat;
			return -1;
		}
...
//printf("submit_control_msg0: status=0x%08x info=0x%x\n", tmp_td[i].status, tmp_td[i].info);
#if 0
	{
		char *sp=(char *)setup;
...
//printf("submit_control_msg1: status=0x%08x info=0x%x\n", tmp_td[i].status, tmp_td[i].info);
		dataptr += pktsze;
		len -= pktsze;
	}
...
//printf("submit_control_msg2: status=0x%08x info=0x%x\n", tmp_td[i].status, tmp_td[i].info);
	/*	usb_show_td(i+1);*/
...
int submit_bulk_msg(struct usb_device *dev, unsigned long pipe, void *buffer,int transfer_len)
{
	...
	static int init_toggle0=0, init_toggle1=0;
	...
	if (!maxsze)
		return -1;
/*
	if( !init_toggle1 && (usb_packetid(pipe) == USB_PID_IN) )
	{
		printf("BULK-IN INIT TOGGLE\n");
		usb_dotoggle (dev, usb_pipeendpoint (pipe), usb_pipein (pipe));
		init_toggle1 = 1;
	}

	wait_ms(1000);
	if( !init_toggle0 && (usb_packetid(pipe) == USB_PID_OUT) )
	{
		printf("BULK-OUT INIT TOGGLE\n");
...
*/
	...
	/* 3 errors */
	status = (pipe & TD_CTRL_LS) | TD_CTRL_ACTIVE | (3 << 27);
	if( (usb_packetid(pipe) == USB_PID_IN) )
	{
		status |= TD_CTRL_SPD;
	}
...
//		printf("DC::submit_bulk_msg: tmp_td[%d] status=0x%08x info=0x%x\n", i-1, tmp_td[i-1].status, tmp_td[i-1].info);
/*
		{
			int j=0;
			//int len = pktsze;
			char *p = (char *)buffer;
			printf("  ");
			for(j=0; j<0x18; j++) printf("%02x ",*(p+j));
			printf("\n");
		}
*/
	} while (len > 0);
...
void usb_check_skel(void)
{
	...
	if(qh_cntrl.dev_ptr!=0) /* it's a device assigned check if this caused IRQ */
	{
		...
		//if( flag_response_disp ) printf("SC=0x%x\n", dev->status );
	...
	if(qh_bulk.dev_ptr!=0) /* it's a device assigned check if this caused IRQ */
	{
		...
		usb_get_td_status(&tmp_td[0],dev); /* update status */
		//if( flag_response_disp ) printf("SB=0x%x\n", dev->status );
		if(!(dev->status & USB_ST_NOT_PROC)) { /* is not active anymore, disconnect devices */
			qh_bulk.dev_ptr=0;
		}
	}
...
{
	static int lpcnt=0;
	//if( lpcnt++%1000==0 ) printf("#");
	//if ( flag_response_disp ) printf("#");

}
	if (!status)		/* shared interrupt, not mine */
	{
		printf("@@@ DC:: NOT MIME INTERRUPT\n");
		return;
	}
	if (status != 1) {
		...
		if ((status&0x20) && ((in16r(usb_base_addr+USBCMD) && USBCMD_RS)==0)) {
			...
			printf("@@@ DC:: REMOVE HC HALDED STATE\n");
		}
	}
...
void Pci_CfgWrite_dword (uint busNum,uint deviceNum,uint funNum,uint regNum,uint data);
unsigned long Pci_CfgRead_dword(uint busNum, uint deviceNum, uint funNum, uint regNum);
/* init uhci
 */
int usb_lowlevel_init(void)
{
	...
	irqvec = temp;
        USB_UHCI_PRINTF("Interrupt Line=%d Pin = %ld\n",irqvec, temp);
	irq_free_handler(irqvec);

        pci_read_config_dword(busdevfunc,PCI_BASE_ADDRESS_4, &usb_base_addr);

        usb_base_addr &= 0xFFFFFFF0;
        usb_base_addr |= PCI_CPU_IO_BASE ;
        printf("usb_base_addr=0x%08x\n", usb_base_addr );

        busno = (busdevfunc & 0xc000) >> 14;
        devno = (busdevfunc & 0x3800) >> 11;
        funcno = (busdevfunc & 0x0700) >> 8;

        Pci_CfgWrite_dword (0, 0, 0, PCI_BASE_1ST, 0);                  //to setup upstream mapping
