...
int flash_erase (flash_info_t * info, int s_first, int s_last)
{
	...
	int prot;
	flash_sect_t sect;
#if defined( CONFIG_CENTAUR ) && !defined( CONFIG_KS8692 )
	flash_sect_t l_sect = -1;
#endif
...
if ( l_sect >= 0 )
{
	int prot;
	ulong start, now, last;
	vu_char* addr = ( vu_char* ) info->start[ l_sect ];
...
static void flash_write_cmd (flash_info_t * info, flash_sect_t sect, uint offset, uchar cmd)
{
	...
	flash_make_cmd (info, cmd, (void*)&cword);
	switch (info->portwidth) {
	case FLASH_CFI_8BIT:
...
#ifdef CONFIG_ARCH_CINTEGRATOR
		/* Value calculated above seems to short for my (old, red) board */
		info->write_tout++;
#endif
