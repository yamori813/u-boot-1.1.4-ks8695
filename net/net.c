...
static uchar   ArpCacheIndex=0;

#if (CONFIG_COMMANDS & CFG_CMD_PING)
static int SearchArpCache(IPaddr_t ip, uchar *macptr)
{
#ifdef DAVIDCHOI_TEST_CODE
        int     index=0;
        IPaddr_t tmp;
        tmp = NetReadIP((void *)&ip);
	if( !ArpCacheIndex ) return -1;
        for(index=0; index < ArpCacheIndex; index++ )
        {
                if ( tmp == ArpCache[index].ip )
                {

                        memcpy(macptr,ArpCache[index].MacEther,06);
                        return (int)NULL;
                }
        }
#endif
...
static int SaveArpCache(IPaddr_t ip, uchar *macptr)
{
        //IPaddr_t tmp = NetReadIP((void *)&ip);
        if ( ArpCacheIndex >= 10 )
                return -1;
        memcpy(ArpCache[ ArpCacheIndex ].MacEther, macptr, 06);
        ArpCache[ ArpCacheIndex ].ip = ip;

#ifdef ET_DEBUG
printf("SaveArpCache:: ip=0x%x mac=0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
        ip, macptr[0],macptr[1],macptr[2],macptr[3],macptr[4],macptr[5]);
#endif
...
#ifdef CONFIG_CENTAUR
	if( SearchArpCache(NetPingIP, ((Ethernet_t *)NetArpWaitTxPacket)->et_dest ) >= 0 )
	{
		int	tcnt,lpcnt=0x100;
		printf("Bypass sending ARP\n");
		for(tcnt=0;tcnt < lpcnt; tcnt++)
...
NetReceive(volatile uchar * inpkt, int len)
{
	...
	case PROT_ARP:
		...
		puts ("Got ARP\n");
#ifdef CONFIG_CENTAUR
		printf("Got ARP::NetOurIp=0x%08x\n",NetOurIP);
#endif
#endif
		arp = (ARP_t *)ip;
		if (len < ARP_HDR_SIZE) {
...
#ifdef CONFIG_CENTAUR
				SaveArpCache(tmp,NetArpWaitPacketMAC);
#endif
		...
		if (ip->ip_p == IPPROTO_ICMP) {
			...
			case ICMP_ECHO_REQUEST:
...

				{
					...
					memcpy(buf,(uchar *)&ip->ip_src, 4);
					memcpy((uchar *)&ip->ip_src, (uchar *)&ip->ip_dst, 4);
					memcpy((uchar *)&ip->ip_dst, buf, 4);
				
					memcpy(buf,et->et_dest, 6);
					memcpy(et->et_dest, et->et_src, 6);
					memcpy(et->et_src, buf, 6);

					ip->ip_sum   = 0;
					ip->ip_sum   = ~NetCksum((uchar *)ip, IP_HDR_SIZE_NO_UDP / 2);

					s = &ip->udp_src;		/* XXX ICMP starts here */
					s[0] = htons(0x0000);		/* echo-reply, code */
					s[1] = 0;			/* checksum */
					payloadlen = NetRxPktLen - ( (uchar *)s - (uchar *)inpkt);
					if ( payloadlen & 1 ) {
						unsigned char* data = ( unsigned char* ) s;
						data[ payloadlen ] = 0;
						++payloadlen;
					}
					s[1] = ~NetCksum((uchar *)s, payloadlen/2);

					(void) eth_send ((uchar *)et, NetRxPktLen);
