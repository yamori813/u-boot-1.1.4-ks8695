...
#define	KS8692_UART_ADDR	((void *) (KS8692_IO_BASE + KS8692_UART1_RX_BUFFER))
#define	KS8692_UART_CLK		25000000
...
void serial_setbrg(void)
{
	...
	volatile struct ks8692uart *uartp = KS8692_UART_ADDR;

	/* Set to global baud rate and 8 data bits, no parity, 1 stop bit*/
	uartp->BD = KS8692_UART_CLK / gd->baudrate;
	uartp->LCR = UART_LINEC_8DATA_BIT;
}
...
void serial_raw_putc(const char c)
{
	...
	for (i = 0; (i < 0x100000); i++) {
		if (uartp->LSR & UART_LINES_TX_HOLD_EMPTY)
			break;
	}
...
int serial_tstc(void)
{
	volatile struct ks8692uart *uartp = KS8692_UART_ADDR;
	if (serial_console)
		return ((uartp->LSR & UART_LINES_RX_READY) ? 1 : 0);
	return 0;
}
...
int serial_getc(void)
{
	...
	while ((uartp->LSR & UART_LINES_RX_READY) == 0)
		;
	return (uartp->RX);
}
